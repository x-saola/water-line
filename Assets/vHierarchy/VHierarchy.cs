...
namespace VHierarchy
{
    ...
    {
        // Converts a tree view selection element to an int instance id. Handles both the legacy `int`
        // representation and Unity 6's `EntityId` struct (which exposes an implicit/explicit int conversion).
        static int ToInstanceId(object o)
        {
            ...
            foreach (var m in t.GetMethods(BindingFlags.Public | BindingFlags.Static))
                if ((m.Name == "op_Implicit" || m.Name == "op_Explicit") && m.ReturnType == typeof(int))
                {
            ...
            try { return System.Convert.ToInt32(o); } catch { return o.GetHashCode(); }
        }

        // In Unity 6 the tree view's id-based methods (FindItem, GetRow, SetExpanded, ...) take an
        // `EntityId` struct instead of `int`. C1onverts an int instance id to whatever id type the
        // resolved method expects (legacy `int`, or `EntityId` via its implicit int conversion).
        static object ToMethodIdArg(Type paramType, int instanceId)
        {
            if (paramType == typeof(int)) return instanceId;

            foreach (var m in paramType.GetMethods(BindingFlags.Public | BindingFlags.Static))
                if ((m.Name == "op_Implicit" || m.Name == "op_Explicit") && m.ReturnType == paramType)
                {
                    var ps = m.GetParameters();
                    if (ps.Length == 1 && ps[0].ParameterType == typeof(int))
                        return m.Invoke(null, new object[] { instanceId });
                }

            return instanceId;
        }

        // Invokes a tree-view method by name, coercing any int instance-id argument to the parameter
        // type the method actually declares (int or Unity 6's EntityId). Unlike the exact-match
        // InvokeMethod helper this also tolerates arguments that are subtypes of the declared parameter
        // (e.g. GameObjectTreeViewItem passed where TreeViewItem is expected).
        static object InvokeTreeMethod(object target, string methodName, params object[] args)
        {
            var type = target.GetType();

            MethodInfo method = null;
            for (var t = type; t != null && method == null; t = t.BaseType)
                method = t.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                          .FirstOrDefault(m => m.Name == methodName && m.GetParameters().Length == args.Length);

            if (method == null)
                throw new System.Exception($"Method '{methodName}' not found in '{type.Name}' type and its parent types");

            var ps = method.GetParameters();
            var converted = new object[args.Length];
            for (int k = 0; k < args.Length; k++)
                converted[k] = (args[k] is int instanceId && ps[k].ParameterType != typeof(int))
                    ? ToMethodIdArg(ps[k].ParameterType, instanceId)
                    : args[k];
        ...
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void setState()
            {
                void set_isRowSelected()
                {
                    ...
                    // In Unity 6 the tree view's drag-selection backing list is List<EntityId>, not List<int>,
                    // so reading it as List<int> throws InvalidCastException. Read it non-generically and
                    // convert each element to an instance id.
                    var dragSelectionList = treeViewController?.GetFieldValue("m_DragSelection")?.GetFieldValue("m_List") as IEnumerable;
#else
                    var dragSelectionList = treeViewController?.GetFieldValue<List<int>>("m_DragSelection") as IEnumerable;
#endif

                    var dragSelectionIds = new HashSet<int>();

                    if (dragSelectionList != null)
                        foreach (var id in dragSelectionList)
                            dragSelectionIds.Add(ToInstanceId(id));

                    var dragging = dragSelectionIds.Count > 0;

                    isRowSelected = dragging ? dragSelectionIds.Contains(go.GetInstanceID()) : Selection.Contains(go);
        ...
        {
            // In Unity 6 expandedIDs is List<EntityId>, not List<int> — read it non-generically and convert.
            var expandedIDsRaw = hierarchyWindow?.GetFieldValue("m_SceneHierarchy")?.GetFieldValue("m_TreeViewState")?.GetPropertyValue("expandedIDs") as IEnumerable;

            expandedIds = new List<int>();

            if (expandedIDsRaw != null)
                foreach (var id in expandedIDsRaw)
                    expandedIds.Add(ToInstanceId(id));
        ...
        // Unity 6 removed TreeViewController.ChangeFoldingForSingleItem(int, bool). Drive the foldout
        // animation through UserInputChangedExpandedState(TreeViewItem, int row, bool) — the same path the
        // controller uses when the user clicks the foldout arrow — resolving the item and row from the
        // data source. Falls back to a plain (non-animated) expand if the item can't be resolved.
        static void SetExpandedWithAnimation(int instanceId, bool expanded)
        {
            var controller = treeViewController;
            var data = controller.GetPropertyValue("data");

            var item = InvokeTreeMethod(data, "FindItem", instanceId);
            if (item == null) { SetExpanded(instanceId, expanded); return; }

            var row = (int)InvokeTreeMethod(data, "GetRow", instanceId);
            InvokeTreeMethod(controller, "UserInputChangedExpandedState", item, row, expanded);
        }
        static void SetExpanded(int instanceId, bool expanded) => InvokeTreeMethod(treeViewController.GetPropertyValue("data"), "SetExpanded", instanceId, expanded);
        static int VisibleRowIndex(int instanceId) => (int)InvokeTreeMethod(treeViewController.GetPropertyValue("data"), "GetRow", instanceId);
