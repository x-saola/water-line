...
namespace Nementic.SelectionUtility
{
	...
	internal sealed class SelectionPopup : PopupWindowContent
	{
		...
		const float parentWOffset = 150f;
		private void DrawRow(Rect rect, Event current, GameObject item)
		{
			...
			rect.width = buttonWidth + parentWOffset;

			var parent = item != null ? item.transform.parent : null;
			var itemName = item != null ? (parent != null ? $"{parent.name}/{item.name}" : $"{item.name}") : "Null";
			GUIContent nameContent = styles.TempContent(itemName, null);
		...
		public override Vector2 GetWindowSize()
		{
			...
			Vector2 windowSize = new Vector2(iconBeforeLabelWidth + buttonAndIconsWidth +  parentWOffset, totalHeight);
