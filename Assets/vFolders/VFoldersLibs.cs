    ...
    public static class VUtils
    {
        ...
        public static object InvokeMethod(this object o, string methodName, params object[] parameters)
        {
            ...
            if (type.GetMethodInfo(methodName, parameters.Select(r => r.GetType()).ToArray()) is MethodInfo methodInfo)
            {
                ...
                var coercedParameters = new object[parameters.Length];
                for (var i = 0; i < parameters.Length; i++)
                    coercedParameters[i] = ConvertElement(parameters[i], methodParams[i].ParameterType);

                return methodInfo.Invoke(target, coercedParameters);
            }
        ...
        static MethodInfo GetMethodInfo(this Type type, string methodName, params Type[] argumentTypes)
        {
            ...
            // Some argument types no longer match exactly across Editor versions (e.g. an int argument where a
            // method now takes the EntityId struct), so GetMethod's exact-type overload lookup above finds nothing.
            // Fall back to matching by name + parameter count; InvokeMethod coerces the actual arguments afterwards.
            for (var curType = type; curType != null; curType = curType.BaseType)
                if (curType.GetMethods(maxBindingFlags).FirstOrDefault(m => m.Name == methodName && m.GetParameters().Length == argumentTypes.Length) is MethodInfo methodInfo)
                    return methodInfoCache[type][methodHash] = methodInfo;

            foreach (var interfaceType in type.GetInterfaces())
                if (interfaceType.GetMethods(maxBindingFlags).FirstOrDefault(m => m.Name == methodName && m.GetParameters().Length == argumentTypes.Length) is MethodInfo methodInfo)
                    return methodInfoCache[type][methodHash] = methodInfo;
        ...
        public static T GetFieldValue<T>(this object o, string fieldName, bool exceptionIfNotFound = true) => ConvertReflectedValue<T>(o.GetFieldValue(fieldName, exceptionIfNotFound));
        public static T GetPropertyValue<T>(this object o, string propertyName, bool exceptionIfNotFound = true) => ConvertReflectedValue<T>(o.GetPropertyValue(propertyName, exceptionIfNotFound));
        public static T GetMemberValue<T>(this object o, string memberName, bool exceptionIfNotFound = true) => ConvertReflectedValue<T>(o.GetMemberValue(memberName, exceptionIfNotFound));
        public static T InvokeMethod<T>(this object o, string methodName, params object[] parameters) => ConvertReflectedValue<T>(o.InvokeMethod(methodName, parameters));

        // Some Unity Editor internals changed the concrete type behind reflected members across versions
        // (e.g. ids became EntityId structs instead of raw ints, expandedIDs became List<EntityId> instead of
        // List<int>), so a direct cast throws InvalidCastException/ArgumentException. Fall back to converting
        // via whatever implicit/explicit operator connects the two types.
        static T ConvertReflectedValue<T>(object value)
        {
                ...
                && typeof(T).GetGenericTypeDefinition() == typeof(List<>))
            {
                ...
                var list = (IList)System.Activator.CreateInstance(typeof(T));
                foreach (var item in enumerable) list.Add(ConvertElement(item, elementType));
                return (T)list;
            }

            return (T)ConvertElement(value, typeof(T));
        }

        // Newer Unity Editor versions wrap raw ints (e.g. instance/asset ids) in structs like EntityId
        // that only convert via an implicit/explicit operator, not a direct cast.
        static object ConvertElement(object item, Type targetType)
        {
            if (item == null || targetType.IsInstanceOfType(item)) return item;

            var sourceType = item.GetType();
            var converter = sourceType.GetMethods(BindingFlags.Public | BindingFlags.Static)
            ...
            if (converter != null) return converter.Invoke(null, new[] { item });

            return System.Convert.ChangeType(item, targetType);
        }
