...
namespace VFolders
{
    ...
    {
        // Newer Unity Editor versions made the Project Browser's tree view generic over EntityId
        // (UnityEditor.IMGUI.Controls.TreeViewItem`1[EntityId]), which is unrelated to the legacy non-generic
        // TreeViewItem class (itself just TreeViewItem`1[int]), so we can no longer cast to it directly.
        // This wraps the raw reflected item and exposes the members we need via VUtils reflection helpers.
        class RItem
        {
            public readonly object Raw;
            RItem(object raw) => Raw = raw;
            public static RItem Wrap(object raw) => raw == null ? null : new RItem(raw);

            public int Id => Raw.GetPropertyValue<int>("id");
            public string DisplayName => Raw.GetPropertyValue<string>("displayName");
            public bool HasChildren => Raw.GetPropertyValue<bool>("hasChildren");
            public RItem Parent => Wrap(Raw.GetPropertyValue("parent"));

            public List<RItem> Children
            {
                get
                {
                    if (!(Raw.GetPropertyValue("children") is IEnumerable rawChildren)) return null;

                    var children = new List<RItem>();
                    foreach (var rawChild in rawChildren) children.Add(Wrap(rawChild));
                    return children;
                }
            }

            public override bool Equals(object obj) => obj is RItem other && ReferenceEquals(Raw, other.Raw);
            public override int GetHashCode() => Raw?.GetHashCode() ?? 0;
            public static bool operator ==(RItem a, RItem b) => ReferenceEquals(a, null) ? ReferenceEquals(b, null) : a.Equals(b);
            public static bool operator !=(RItem a, RItem b) => !(a == b);
        }
        ...
        static void RowGUI(string guid, Rect rowRect)
        {
            ...
            void setObjects()
            {
                ...
                void set_treeItem_byRect()
                {
                    ...
                    treeItem = RItem.Wrap(tree.data.InvokeMethod("GetItem", rowIndex));
                ...
                void set_treeItem_byInstanceId()
                {
                    ...
                    treeItem = RItem.Wrap(tree.treeViewController.InvokeMethod("FindItem", instanceId));
            ...
            void drawing()
            {
                ...
                void hierarchyLines()
                {
                    ...
                    bool isLastChild(RItem item) => item.Parent?.Children?.LastOrDefault() == item;
                    bool hasChilren(RItem item) => item.Children != null && item.Children.Count > 0;
        ...
        static RItem lastHoveredTreeItem;
        ...
        class Tree
        {
            ...
            public bool IsExpanded(RItem treeItem) => expandedIds.Contains(treeItem.Id);
